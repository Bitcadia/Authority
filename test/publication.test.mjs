...
test("generated publications preserve entries, pins, signatures and reproducibility", async t => {
  ...
  assert.deepEqual(await validatePublication(root, output, { requireSigned: true }), { entries: 1568, signed: true });
  ...
  assert.equal((await validatePublication(root, unsigned)).entries, 1568);

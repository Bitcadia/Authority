...
test("Amped Up keeps mirror and verified newer RAR release distinct", async () => {
  ...
  assert.equal(latest.status, "verified-client-rar");
  ...
  const rar = registry.entries.find(entry => entry.id === evidence.rarCandidate.id);
  assert.deepEqual(rar, evidence.rarCandidate);
  assert.equal(rar.patch.sha256, latest.artifact.sha256);
  assert.equal(rar.patch.memberSha256, latest.member.sha256);
  assert.equal(rar.output.sha256, latest.output.sha256);
  assert.equal(rar.source.provider, "GameBanana");
});
